    ...
    def vs (self, v, options = {}, plot_args = {}):
        ...
        if options["fitlm"]:
            ...
            for i in range(data[0].shape[1]):
                a = min(data[0][:,i] - data[2][:,i])
                b = max(data[0][:,i] + data[2][:,i])
                e = (b-a)/10

                f = lambda t: m.value[i]*t + n.value[i]
                    
                plt.plot([a-e,b+e], [f(a-e),f(b+e)], color = f"C{i}")
                plt.errorbar(data[0][:,i], data[1][:,i], data[2][:,i], data[3][:,i], color = f"C{i}", **plot_args)
